...
if( ( stf_get_option('related') && empty( $get_meta["stf_hide_related"][0] ) ) || ( isset( $get_meta["stf_hide_related"][0] ) && $get_meta["stf_hide_related"][0] == 'no' ) ):
	$related_no = stf_get_option('related_number') ? stf_get_option('related_number') : 3;
	
	global $post;
	$orig_post = $post;
	
	$query_type = stf_get_option('related_query') ;
	...
	<section id="related_posts">
		<div class="block-head">
			<h3><?php if(stf_get_option('related_title')) echo stf_get_option('related_title'); else '相關文章'; ?></h3><div class="stripe-line"></div>
		</div>
		<div class="post-listing">
			<?php while ( $related_query->have_posts() ) : $related_query->the_post()?>
			<div <?php stf_post_class('related-item'); ?>>
				...
				<p class="post-meta"><?php stf_get_time() ?></p>
			</div>
			<?php endwhile;?>
			<div class="clear"></div>
		</div>
