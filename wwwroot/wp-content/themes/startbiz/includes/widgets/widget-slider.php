...
*/
add_action( 'widgets_init', 'stf_slider_widget' );
function stf_slider_widget() {
	register_widget( 'stf_slider' );
}
class stf_slider extends WP_Widget {

	function stf_slider() {
		$widget_ops = array( 'classname' => 'stf-slider' );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'stf-slider-widget' );
		$this->WP_Widget( 'stf-slider-widget',theme_name .' - 幻燈片', $widget_ops, $control_ops );
	}
	...
	<?php endif; ?>
	<?php else :
		$custom_slider_args = array( 'post_type' => 'stf_slider', 'p' => $custom_slider, 'no_found_rows' => 1  ) ;
	...
	<div class="flexslider" id="<?php echo $args['widget_id']; ?>">
		...
		<?php while ( $custom_slider_query->have_posts() ) : $custom_slider_query->the_post();
			...
			<li>
				...
				<img src="<?php echo stf_slider_img_src( $slide['id'] , 'stf-large' ) ?>" alt="" />
	...
	function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'cats_id' ); ?>">分類 : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'custom_slider' ); ?>">自訂幻燈片 : </label>
			...
			<?php else: ?>
			<span style="color:#FF0000;">新增第一個自訂幻燈片</span>
			<?php endif; ?>
		</p>
	<?php
	}
