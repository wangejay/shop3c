...
*/
add_action( 'widgets_init', 'stf_latest_tweet_widget' );
function stf_latest_tweet_widget() {
	register_widget( 'stf_Latest_Tweets' );
}
class stf_Latest_Tweets extends WP_Widget {

	function stf_Latest_Tweets() {
...
<?php			}
		...
		echo $before_title; ?>
			<a href="http://twitter.com/<?php echo $twitter_username  ?>"><?php echo $title ; ?></a>
		<?php echo $after_title; 
		echo '請在外觀 > 主題選項 > 設定 Twitter API OAuth';
		echo $after_widget;
	}
}
	...
	function form( $instance ) {
		$defaults = array( 'title' => '@ 追蹤我們' , 'no_of_tweets' => '5' );
		...
		if( empty($twitter_username) || empty($consumer_key) || empty($consumer_secret) || empty($access_token) || empty($access_token_secret)  )
				echo '<p style="display:block; padding: 5px; font-weight:bold; clear:both; background: rgb(255, 157, 157);">錯誤 : 請在外觀 > 主題選項 > 設定 Twitter API OAuth</p>';
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_tweets' ); ?>">推訊（Tweets）顯示數量 : </label>
